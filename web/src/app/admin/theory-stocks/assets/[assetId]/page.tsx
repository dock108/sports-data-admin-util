...
import {
  fetchEquityAsset,
  listEquityCandles,
  type EquityAssetDetail,
  type EquityCandleSummary,
} from "@/lib/api/stocksAdmin";

export default function StocksAssetDetailPage() {
  const params = useParams();
  const assetId = Number(params.assetId);

  const [asset, setAsset] = useState<EquityAssetDetail | null>(null);
  const [candles, setCandles] = useState<EquityCandleSummary[]>([]);
    ...
    async function load() {
      try {
        ...
        const [assetResp, candlesResp] = await Promise.all([
          fetchEquityAsset(assetId),
          listEquityCandles({ assetId, limit: 100 }),
        ]);
        setAsset(assetResp);
  ...
  return (
    <div className={styles.container}>
      <Link href="/admin/theory-stocks/assets" className={styles.backLink}>
        ← Back to assets
      </Link>
