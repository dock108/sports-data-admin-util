# Changelog

## Unreleased
- Added compact play-by-play slice endpoint for admin games.
- Improved compact moment summary logging and fallback handling.
- Added summary cache hit-rate tracking and spoiler redaction logging for compact summaries.
- Split sports admin router modules and scraper helpers to keep files under the size target.
- Consolidated sports admin API client types into modular helpers.
- Moved root docs into /docs and added a local development guide.
